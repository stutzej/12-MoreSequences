...
def run_test_generate_points_on_circle():
    ...
    # -------------------------------------------------------------------------
    # Test 2:
    expected = [rg.Point(0, 1.0),  # All numbers are approximate.
                rg.Point(1, 0),
                rg.Point(-1, 0),
                rg.Point(0, -1)]
...
def pizza(window, circle, number_of_slices, color, thickness):
    ...
    for k in range(number_of_slices):
        new_lines = rg.Line(circle.center, new_points[k])
        new_lines.color = color
        new_lines.thickness = thickness
        new_lines.attach_to(window)
        window.render()
...
def run_test_polygon():
    ...
    # -------------------------------------------------------------------------
    # Test 3:
    title = ('POLYGON test 3: 5 segments with thick cyan lines')
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 80)
    circle.outline_thickness = 3
    polygon(window, circle, 5, 'cyan', 10)
    window.close_on_mouse_click()
...
def polygon(window, circle, number_of_segments, color, thickness):
    ...
    for k in range(number_of_segments):
        ...
        new_lines = rg.Line(new_points[k], new_points[end])
        new_lines.color = color
        new_lines.thickness = thickness
        new_lines.attach_to(window)
        window.render()
...
def run_test_fancy_polygon():
    ...
    # -------------------------------------------------------------------------
    # Test 4:
    title = ('FANCY POLYGON test 4:  20 cyan lines, hops = 5')
    window = rg.RoseWindow(400, 400, title)

    circle = rg.Circle(rg.Point(200, 200), 100)
    fancy_polygon(window, circle, 20, 5, 'cyan', 3)
    window.continue_on_mouse_click()
...
def fancy_polygon(window, circle, number_of_lines, hops_to_next_point, color,
    ...
    for k in range(number_of_lines):
        index = k + hops_to_next_point
        if index >= number_of_lines:
            end = new_points[index - number_of_lines]
        else:
            end = new_points[k + hops_to_next_point]
        start = new_points[k]
